...
**(a) 첫 세션이면 — `onboard.sh` 한 줄로 워크트리 발급 ★**

네가 *처음* 합류해서 `~/Desktop/code/personal/AIL/<네 이름>/`이 아직 없으면, 어느 워크트리(보통 `arche`)에서든 한 줄:

```bash
cd ~/Desktop/code/personal/AIL/arche
bash community-tools/onboard.sh <네 이름>     # 예: tekton, homeros
```

이 스크립트가 자동으로:
1. `~/Desktop/code/personal/AIL/<네 이름>/` 워크트리 생성 (브랜치가 origin에 있으면 그걸 checkout, 없으면 `origin/dev`에서 분기)
2. `extensions.worktreeConfig=true` + `ail.identity=<네 이름>` (per-worktree) + `core.hooksPath=.githooks` 박음
3. `origin/dev`에 자동 rebase

멱등 — 이미 워크트리가 있으면 config 갱신만 하고 끝난다. 끝난 뒤 새 세션을 거기서 spawn하거나 `cd ~/Desktop/code/personal/AIL/<네 이름>`으로 이동.

**(a') 이미 워크트리가 있는 멤버는 바로 이동만** ([CLAUDE.md Rule 4](CLAUDE.md#rule-4--브랜치-전략))
...
메인 repo(`AIL/arche`)는 arche가 산다 — 거기서 작업하면 같은 머신의 다른 세션과 브랜치가 충돌한다.

**(b) (자동화로 박힌 것 점검 — 보통 생략)** ([CLAUDE.md Rule 4](CLAUDE.md))

`onboard.sh`가 다 박았으면 이 단계는 점검만으로 끝난다. 직접 손으로 박을 일이 있을 때만:

```bash
git config core.hooksPath .githooks          # dev/main 직접 커밋 차단 hook
git config extensions.worktreeConfig true    # per-worktree 설정 분리 (먼저)
git config --worktree ail.identity <네 이름> # 반드시 --worktree
...
브랜치가 origin에 없으면 `onboard.sh`가 `origin/dev`에서 분기해서 만든다 — 첫 push 시 자동으로 `origin/<네 이름>` 생성.
