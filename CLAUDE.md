...
**버전:** v1.71.1 main + tag + PyPI. 테스트 **867 collected** (evolve-server subprocess 3개 CI skip — 로컬에서만 실행).
...
**현재 상태:** v1.71.1 main + PyPI 배포 완료. 867 collected. **Polis 마일스톤 6개 전부 main 라이브** (#1~#6 ✅). Stoa Phase A/B/C + stoa-mcp SSE + Channel + UserPromptSubmit hook 복원. v1.70.x는 schedule/deploy/UI 후속 버그픽스 라인. 2026-05-07 cross-team doctrine D1~D3 + 채널 페어링 land (Rule 16, dev `e3868b9`).

**버그 발생 시 진단:** `.ail/chat_history.jsonl` + `.ail/ledger.jsonl` 직접 확인.

**Polis 마일스톤 — 모두 land (Arche 2026-04-27 letter `msg_1777273204_0`):**

- ✅ #1 `on_compact` convention (`spec/04-evolution.md §11a`)
- ✅ #2 `context trust_level` (`spec/02-context.md §9a`, executor.py:513~)
- ✅ #3 `intent is_safe` (auto 모드 자동 안전 분류)
- ✅ #4 `deny-first` effect 정책 (`spec/05-effects.md §11a`, `docs/PRINCIPLES.md §3d`, `tests/test_deny_first.py`)
- ✅ #5 `human.approve` 가이드라인 (`docs/PRINCIPLES.md §3a`)
- ✅ #6 `human.approve` ↔ Stoa 통합 (UI + Stoa 두 채널 race, `notify: [Text]` kwarg, `AIL_APPROVE_TIMEOUT_S`. `tests/test_human_approve_stoa.py` 6 + `tests/test_human_approve.py` 8.)

**그 외 대기:**

0. **🛡 Sphinx — Stoa/Mneme 인증/필터 (hyun06000 2026-04-27 LinkedIn 공유 후 + ID/PW 명시 요구)** — 노출 표면: from 위조, 인박스 공개 읽기, compose 공개 쓰기, agent registry 위조. **단일 write 토큰은 부족** — 토큰 가진 누구나 누구 이름으로든 발송 가능 (impersonation). **per-identity credential (ID/PW식)** 필수. 인간도 AI도 다 인증 대상. **L1 설계** (당장 아님): `{name, password_hash, role}` 등록부 + `X-Stoa-From` + `X-Stoa-Auth` 헤더 + body.from === header.from 검증 + admin 토큰으로 신원 부트스트랩. 읽기는 일단 공개 유지 (게시판 본성). **L2** (그 후): Sphinx-Polis로 분리 + 키 회전 + 역할(admin/agent/guest). Mneme 동일 게이트로 보호. **trigger**: 첫 spam / impersonation 시도 / hyun06000 신호.
1. **HEAAL boundary 확장** — GPT 4종 완료 ✅. 다음: Gemini Pro (API 키 준비 중).
2. **v7 훈련 재시도** — `ollama stop <model>` 선행 + `max-seq-length=1024` 필수. 2회 OOM 이력. Telos 담당.
3. **executor.py 분할** — RFC `docs/proposals/executor-split.md`. 1단계(util 함수 9종 → `executor_utils.py`) Telos 2026-04-30 land. 다음: effects/intents/expressions mixin 분리.
4. **L3 HEAAOS 착수** — L1 해외 검증 후. `os/00-noos.md` 4종 리브랜딩.
5. **Mneme 정식화** — will이 Stoa 인박스에 묻히는 문제 (실증됨). pinned/latest-wins 메커니즘 설계.
6. **Stoa-filed AIL primitive 2건** (2026-05-07 review pass 완료, Mneme-Walter +1 후 Brandon이 `gh issue create --repo hyun06000/AIL` 발사) — `schedule.sleep(seconds: Number) -> Result[Boolean]` (long-poll·throttle 합성), `state.list_keys(prefix: Text) -> Result[[Text]]` (retention·subscriber iteration). 본문: `hyun06000/Stoa@docs/ail-issues/`. arche review msg_1778150707_5, Telos T1 review msg_1778152115_1. 발사 후 Telos가 reference-impl PR (7일 안). Spec patch 권고 4건(A2 shutdown 순서·A4 outer timeout·C1 sleep(0) 정당화·B-cost SQLite 후속 RFC)은 발사 본문에 반영 또는 첫 코멘트 박기.
7. **`state.*` SQLite/LMDB backing 마이그레이션** (Telos T1에서 file 의향 표명) — 의미론 변화 0, backing만 교체. `state.list_keys` retention 워커가 production 부담 만나면 swap. 별 RFC 후속.
8. **`schedule.sleep_until` + `state.delete_prefix` 후속 issue** — `schedule.sleep` + `state.list_keys` land 후 별 사이클에 file. wake-at-absolute-time + atomic delete-by-prefix.
9. **Mneme argon2id builtin issue** (Mneme-Brandon → AIL repo, 본문 finalize 단계, 24h SLA) — primitive 시그니처 review.
10. **Sphinx scope 한 줄 letter** (Ergon → Stoa-Brandon, cc Stoa-Admin·arche) — RFC-002 Phase B (에이전트 ed25519)와 슬롯 충돌 정합. 우리 잠정 정렬: 슬롯1(에이전트=ed25519, Sphinx 추가 layer 0) / 슬롯2(사람=토큰, Sphinx ID/PW가 사람용이면 alternate path) / 슬롯3(에이전트 fallback=Sphinx ID/PW가 키 분실·rotation 폴백이면 별 슬롯). Ergon이 본 시점 *대기 중* 트랙임 명시.
11. **Stage B 사이클 진입 신호 대기** (2~3 사이클 후, Stoa 서버 측 RFC-002 Phase B + RFC-004 Phase C 동시 점화) — AIL agent들이 사이드카 stoa-cli로 envelope 서명 → POST. AIL 본체 추가 작업 0. 키 vault path는 Mneme RFC-001 §5(per-identity, INSERT-only, latest-wins) 권고 채택.
12. **Stoa 트래픽 monitoring 사안** (2026-05-07 incident 후 Stoa-Admin 합의) — (1) `human.approve` plan size 8KB soft cap (`executor.py` `_stoa_post_approval`, ~10 LOC, Telos 영역) — Marcus 진단이 "AIL plan letter 큰 거 있다" 짚을 때 즉시 land, 본 사이클은 신호 0으로 보류. (2) pre-push hook broadcast 평소 ~1KB OK / monorepo merge 시 spike 가능, monitoring만. (3) community-tools 자동 발송 도구(session_end/universal_agent/stoa_hello) spam guard — 무한 루프·조건 미스 시 mass insert 위험. **Stoa 측 hotfix `58f0db1`(retention 7일 + 100KB cap) land 후 follow-up 사이클**.
13. **Stoa traffic audit 도구** — Ergon에 위임 (community-tools 영역). sender별 count/총량/평균/max 분포 + 전체 stats. incident 진단을 모두가 동일 도구로. AIL Rule 9 정합 (HTTP·parse_json만 사용, Python 의존 없음). Arche가 본 incident 사이에 손으로 한 audit 결과(`msg_1778156389_11`) 도구화.
