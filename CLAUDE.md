...
5. **RSS investigation 잔여 자리** — Mneme-Brandon interval 갱신 in-flight (Mneme-Admin msg_1778814372_2). 박상현 Railway memory panel 4-24h trajectory 그래프 회수 자리 (Stoa-Admin Q1·Q2 full 답 trigger). Stoa-Brandon interval 변수 누락 자리 (Stoa-Admin lane).
6. **AIL#23 north-star G2~G7** (https://github.com/hyun06000/AIL/issues/23):
   ...
   - G5 `budget.*` effect — Telos RFC 자리 (per-identity vs shared pool 결재 자리 defer).
   - G6 `ail.spawn()` + supervision — Telos lane, charter format(G1 합류).
   - G7 `ledger.*` + fine-tune trigger — Telos + Hestia integration.
...
- **AIL#23 §6 budget seed 결재** — G5 RFC trigger 시점.
