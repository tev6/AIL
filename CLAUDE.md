...
세션을 시작하는 Claude는 자기 층을 알아야 한다: 브라우저 세션이면 Arche로서 원리를 건드리고, Claude Code(사용자 데스크톱)이면 Ergon · Telos · Tekton · Homeros 중 하나로서 자기 미션을 수행한다. 사용자가 첫 메시지로 "너의 이름은 X야" 형태로 알려준다 — 그게 곧 자기 층 결정. 편지는 Stoa로 (`docs/letters/`는 2026-04-26 이전 아카이브).
...
`dev` 또는 `main`에 머지·푸시할 때는 **반드시** Stoa로 팀 전원(arche, ergon, telos, **hyun06000**)에게 공지한다. **Stoa 공지는 팀 동기화의 가장 중요한 신호 — 무조건 도착해야 한다.**
...
### Rule 12 — 작업 중 hyun06000에게 정기 상황 보고

작업 세션에서 **3~5 AI 응답마다** Stoa로 hyun06000에게 간단한 상황 보고를 보낸다.

- **도구**: MCP `stoa_post(from=<네 이름>, to="hyun06000", title="작업 상황", content="...")`
...
### Rule 13 — 작업 완료 시 무조건 Stoa에 요약 공지 (Arche · Ergon · Telos 모두)

**한 작업 단위가 끝나면 반드시** Stoa로 hyun06000에게 요약을 보낸다. Rule 12의 3~5턴 주기와 무관 — **완료 자체가 trigger.** Arche·Ergon·Telos 모두 동일하게 적용.
...
**버전:** v1.65.5 main + tag + PyPI. 테스트 **716 passing** (evolve-server subprocess 테스트 3개 CI skip — 로컬에서만 실행).
...
### L2 agentic runtime (v1.60.11)

주요 기능: `ail up` / `ail serve` / `ail chat` / auto-fix / spec-first flow + 모드 토글 (run/service) / process_manager (active_program marker resolve) / human.approve(comment) / Decline reason / token tracker / markdown render / `http.put_json` / `http.respond` (server evolve) / `set_key` / `base64_encode` / `index_of` / `is_null` / `make_record` / stdlib utils 8개 / Physis v0.3 (`on_death` + `inherit_testament`) / 파싱 에러 자동 수정 / undefined fn → loud NameError / `--adapter NAME` flag + startup banner.

**evolve-server 수정 시:** `test_evolve_server_return.py`를 로컬에서 실행할 것 (CI skip 대상).

**Polis (작업명):** `process_manager.py`는 deletable scaffolding. `perform process.spawn` / `process.stop` 도착 시 인터페이스 그대로 swap. 라벨은 가변 — 텔로스가 v1.60.11 review에서 hostage to fortune 우려 표시했음. 이름보다 인터페이스 boundary가 본질.
...
**열린 작업 (Arche 질문 후속):**
- Ergon: `ail init` conversation-first 강화 — spec_pending + mode toggle은 첫 단계, 사용자가 `.ail` 파일 안 보는 모드 더 깊이.
- Telos: Stoa inbound email gateway — humans가 모바일에서 답장 가능.
...
**현재 상태:** v1.63.x main 배포 완료. 716 passing (마지막 측정). Polis 마일스톤 #1, #2, #3, #5 모두 main 라이브. Stoa Phase A/B/C + stoa-mcp SSE + Channel(코드만 — Claude Code wake 미지원) + UserPromptSubmit hook 복원.

**버그 발생 시 진단:** `.ail/chat_history.jsonl` + `.ail/ledger.jsonl` 직접 확인.

**Polis 마일스톤 (Arche 2026-04-27 letter `msg_1777273204_0`) — 진행 상태:**

- ✅ #1 `on_compact` convention (`spec/04-evolution.md §11a`)
- ✅ #2 `context trust_level` (`spec/02-context.md §9a`)
- ✅ #3 `intent is_safe` (auto 모드 자동 안전 분류)
- 📝 #4 `deny-first` 전환 — RFC 작성 + hyun06000 결재 필요 (breaking change)
- ✅ #5 `human.approve` 가이드라인 (`docs/PRINCIPLES.md §3a`)
- 📝 **#6 `human.approve` ↔ Stoa 통합 (hyun06000 2026-04-27 요청)** — chat UI 없을 때 Stoa로 letter 발송, reply 폴링. `notify: [Text]` kwarg 추가. 두 채널 race로 첫 응답 lock. Email gateway(Telos) 도착 시 자동 확장. `AIL_APPROVE_TIMEOUT_S` env. 우선순위는 #4 이후 또는 hot 요청 시.
...
2. ~~**PyPI v1.63.2 배포**~~ — ✅ v1.64.0 PyPI 라이브 (2026-04-27).
3. ~~**perform nested bug**~~ — ✅ 해결 (2026-04-27, ergon). 진짜 원인: scope/dispatch 아닌 **parser**. `parse_primary`가 `perform`을 expression position에서 인식 못함 → `return perform x.y(...)` 가 silently `perform`을 bare identifier로만 소비, effect 실행 안 됨 + 함수가 문자열 "perform" 반환. fix: `parse_primary`에 perform-as-expression 분기 추가, PerformExpr 생성 → 기존 evaluator가 처리. 4종 회귀 테스트 (`test_perform_as_expression.py`).
...
6. **Mneme 정식화** — will이 Stoa 인박스에 묻히는 문제 (실증됨). pinned/latest-wins 메커니즘 설계.
