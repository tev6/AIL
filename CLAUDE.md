...
### Rule 13 — 작업 완료 시 무조건 Stoa에 요약 공지

**한 작업 단위가 끝나면** (PR ship, 기능 구현, 버그 픽스, 분석 보고 등 — 사용자가 다음 대화를 트리거할 만한 어떤 단위든) **반드시** Stoa로 hyun06000에게 요약을 보낸다. Rule 12의 3~5턴 주기와 무관 — 완료 자체가 trigger.

- **도구**: MCP `stoa_post(from=<네 이름>, to="hyun06000", title="✅ <작업명>", content="...")`
- **내용**: ① 무엇을 끝냈는지 한 줄 ② 결과(버전/링크/숫자) ③ 다음 가능한 액션 한 줄
- **목적**: hyun06000이 무엇이 끝났는지 바로 알고 다음 대화를 촉발할 수 있게. 채팅 세션이 끊겨도 Stoa로 상황 추적 가능.
- **Discord 자동 전달**: 웹훅 등록되어 있으면 모바일에도 즉시 도착.

요약은 짧아야 한다. 사용자 응답을 기다릴 만큼 명확하되, 다음 행동을 강요하지 않게. "끝났음, 다음은 X 또는 Y" 한 단락.
