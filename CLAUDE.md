...
**현재 상태:** v1.71.2 main + PyPI 배포 완료 (사이클 6 closing). 881 collected (Telos `schedule.sleep` + `state.list_keys` 14 tests 추가). **Polis 마일스톤 6개 전부 main 라이브** (#1~#6 ✅). Stoa Phase A/B/C + stoa-mcp SSE + Channel + UserPromptSubmit hook 복원. 2026-05-07 cross-team doctrine D1~D3 + 채널 페어링 land (Rule 16). 2026-05-08 D4·D5·D6 doctrine land (Rule 17·18·19, HEAAL audit 정합).
...
**등대가 비추는 결정 자리** (NEXT는 forward-looking only — Rule 7. 현 사이클에서 결정 필요한 항목):
...
6. **v1.72.0 release cut** (D4 substrate gate, Rule 17) — `schedule.sleep` + `state.list_keys` land됨(987b16b). 양 팀 *실 사용* 신호(Stoa/Mneme production import) 또는 24h propagation 도착 시 cut. arche가 cut 자리 본인.
7. **HEAAL audit P3·P5 후속** (Telos 영역) — (P3) `authoring_chat.py` 슬림화 목표 ≤ spec × 1.5 (현 2.7×) — Rule 19 정합. (P5) `auto` mode + is_safe 미정의 default 검토 (V4 정합) — breaking 0 영향 분석 후 결정.
8. **Tekton 영입 신호** (D5 정합, Rule 18) — Go runtime effect 0건 drift 해소의 유일한 path. arche가 별 letter로 박상현에게 영입 결재 letter 발사.

---

**trigger 자율** (도착 신호 받는 멤버가 직접 land — NEXT 결정 자리 아님):

- **`state.*` SQLite/LMDB backing 마이그레이션** (Telos T1에서 file 의향 표명) — 의미론 변화 0, backing만 교체. `state.list_keys` retention 워커가 production 부담 만나면 swap. 별 RFC 후속.
- **`schedule.sleep_until` + `state.delete_prefix` 후속 issue** — 별 사이클에 file. wake-at-absolute-time + atomic delete-by-prefix.
- **Mneme argon2id builtin issue (#8)** (Mneme-Brandon → AIL repo, 본문 finalize 단계) — primitive 시그니처 review. Telos가 issue 코멘트로 review.
- **Sphinx scope 한 줄 letter** (Ergon → Stoa-Brandon, cc Stoa-Admin·arche) — RFC-002 Phase B (에이전트 ed25519)와 슬롯 충돌 정합. 우리 잠정 정렬: 슬롯1(에이전트=ed25519, Sphinx 추가 layer 0) / 슬롯2(사람=토큰, Sphinx ID/PW가 사람용) / 슬롯3(에이전트 fallback=키 분실·rotation 폴백). Ergon 대기 중 트랙.
- **Stage B 사이클 진입 신호 대기** (2~3 사이클 후, Stoa 서버 측 RFC-002 Phase B + RFC-004 Phase C 동시 점화) — AIL agent들이 사이드카 stoa-cli로 envelope 서명 → POST. AIL 본체 추가 작업 0. 키 vault path = Mneme RFC-001 §5 (per-identity, INSERT-only, latest-wins).
- **Stoa monitoring 후속** (Marcus 진단 신호 시) — `human.approve` plan size 8KB soft cap (`executor.py` `_stoa_post_approval`, ~10 LOC, Telos 영역). 본 사이클 신호 0이면 보류 유지. pre-push hook broadcast / community-tools 자동 발송 spam guard는 monitoring만.

---

**최근 close된 항목** (자취):

- ✅ Stoa-filed AIL primitive 2건 — `schedule.sleep` + `state.list_keys` (Telos 987b16b, 2026-05-08).
- ✅ Stoa traffic audit 도구 `community-tools/stoa_audit.ail` (Ergon d1639fd, 2026-05-07).
