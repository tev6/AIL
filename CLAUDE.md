...
**버전:** v1.70.5 main + tag + PyPI. 테스트 **859 collected** (evolve-server subprocess 3개 CI skip — 로컬에서만 실행).
...
### L2 agentic runtime (v1.70.5)

CLI: `ail up` / `ail serve` / `ail chat` / `ail run` / `ail bundle` / `ail doctor` / `ail init` (7개로 정리, v1.70.0 rebuild에서 INTENT.md 제거).

핵심 effects: `clock.now` / `state.*` / `env.read` / `schedule.every` (action override + self-throttle) / `queue.*` (push/take/done/retry) / `http.*` (get/post/put/respond/graphql) / `gh.*` (run/pr/issue) / `git.*` (commit/push/pull) / `mneme.*` (save/load/log) / `db.*` / `email.send` / `image.embed` / `human.approve` (Stoa notify 통합 대기 — Polis #6).
...
**현재 상태:** v1.70.5 main 배포 완료. 859 collected (마지막 측정). Polis 마일스톤 #1, #2, #3, #5 모두 main 라이브. Stoa Phase A/B/C + stoa-mcp SSE + Channel(코드만 — Claude Code wake 미지원) + UserPromptSubmit hook 복원. v1.70.x는 schedule/deploy/UI 후속 버그픽스 라인 (현장 사용 신호).
...
3. **executor.py 분할** — RFC `docs/proposals/executor-split.md` 검토 → 단계적 mixin 추출. 회귀 위험 적은 effects/builtins부터.
