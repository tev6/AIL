...
### Core 1차 목록 (12 entries / 6 scopes — shipped-truth 정합, Telos review 2026-05-14)
...
신규 planned core effect (`clock.epoch_ms`·`file.delete`·`file.exists`·`log.*`)는
executor dispatch 자리가 land된 turn에 yaml entry가 동시 추가된다 — *yaml = shipped truth only*.

### Substrate 1차 목록 (scope 단위)

```
http.*       gh.*        git.*       mneme.*
db.*         email.*     queue.*     secrets.*
human.approve            image.embed              search.web
stoa.*       (Sphinx 인증 land 후 정식 표면)
```

**`crypto.*`은 effect가 아니라 builtin** (Rule 16 D2 — primitive ed25519 sign/verify/keygen,
random_bytes; executor.py:4382-4513 builtin dispatch). 별 `spec/builtins.canonical.yaml`로
사이클 11+ RFC 자리. canonical envelope·서명 owner = Stoa 사이드카.
...
- name: ail.run
  ...
  since: "1.20.0"
  # 자식 context 명시 시 부모 `allow_effects`와 교집합 (§9b 정합).
...
### Static gate (양방향 — Telos review 2026-05-14)
- `cargo test conformance::static` / `go test ./conformance/static` / `pytest test_conformance_yaml_to_executor.py`
- **방향 (a) yaml → runtime**: spec yaml의 모든 effect가 자기 런타임 effect table에 등록됐는지 + 시그니처 일치 + tier 일치. *Boot fails*.
- **방향 (b) runtime → yaml**: 런타임 dispatch 자리에 존재하지만 yaml에 entry가 없는 effect 0 강제. *dead spec / phantom yaml entry 동시 차단*.
- 양방향이 모두 green일 때만 빌드 통과 — yaml = shipped truth 1:1.
...
| Phase | 사이클 | 자취 |
|---|---|---|
| 0 | 10 | D7 doctrine + 본 RFC + yaml = shipped-truth (12 core / 30 substrate) land. **본 사이클**. |
