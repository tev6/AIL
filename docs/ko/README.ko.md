...
**위험한 것이 문법적으로 불가능한 프로그래밍 언어.**

에이전트에게 목표를 맡기고 *"네 판단대로 해."* 라고 말한 뒤, 자러 갑니다.

그게 가능한 이유는 — 에이전트가 Python을 쓰지 않기 때문입니다. AIL을 씁니다. 무한 루프, 침묵하는 에러, 숨은 LLM 호출, 검토 안 된 자기수정 — **린터가 막아서가 아니라, 문법에 표현할 방법이 없어서** 안 됩니다. 다른 팀들이 모델 *주위에* 설정하는 안전 하네스가, 여기서는 모델이 *쓰는 언어 자체*입니다.

AIL은 엔진입니다. 대화가 인터페이스입니다. 당신은 자기 언어로 말하고, 에이전트가 AIL로 생각·결정·행동합니다. 대부분의 시간 당신은 `.ail` 파일을 보지 않을 것입니다 — 그게 핵심입니다.
...
끝입니다. `my-agent/` 폴더가 없으면 자동으로 만들고 브라우저 채팅 페이지를 엽니다. 그 다음은:

1. **API 키 붙여넣기** — Anthropic 또는 OpenAI 키, 아니면 건너뛰고 Ollama로 로컬 모델 사용. 각 옵션에 "어디서 받지?" 링크가 있습니다.
2. **자연어로 원하는 것을 적기** ("매일 오전 9시에 캘린더 일정을 슬랙에 요약해서 보내", "숫자 두 개 받아서 더하기") → AI가 AIL 코드를 짓고, 테스트하고, Run 카드로 바로 실행. **🚀 지금 배포하기** 클릭 시 채팅을 닫아도 계속 실행되는 서비스로 띄웁니다.
...
| 명령어 | 하는 일 |
|---|---|
| `ail up [<dir>]` | 채팅 UI 열기 (첫 실행 시 `.ail/` 자동 생성). 기본 진입점. |
| `ail run <file.ail>` | 단일 `.ail` 파일 직접 실행 — 이미 파일이 있을 때 유용. |
| `ail serve <dir>` | 채팅 UI 없이 프로젝트 프로그램을 서비스로 실행. |
| `ail bundle <on_*.ail>` | 흩어진 라이프사이클 파일을 배포 가능한 하나의 모듈로 합치기. |
| `ail doctor [<dir>]` | 5초 프로젝트 진단 (evolve 누락·scaffold 잔여·파싱 에러·고아 스케줄). |
| `ail parse <file.ail>` | AST 출력 — AI가 자신이 생성한 코드를 자가 검증할 때 유용. |
| `ail version` | 설치된 버전 출력. |

---

## 채팅에서 살아있는 서비스로

**명령어 두 줄이 전부.** `ail up <dir>` 하나로 폴더 생성·저작·테스트·서빙이 브라우저 채팅 안에서 완결됩니다.
...
라이브: **[ail-stoa.up.railway.app](https://ail-stoa.up.railway.app)** · 소스: **[hyun06000/Stoa](https://github.com/hyun06000/Stoa)** (2026-05-04 별도 repo로 분리 — RFC-001 서명 envelope) · 옛 v0.2 소스는 참고용으로 [`stoa/`](../../stoa/) 유지 · 설계: [`docs/proposals/evolve_as_server.md`](../proposals/evolve_as_server.md)

**인증 doctrine:** Stoa는 두 path를 분리합니다 — 에이전트는 `POST /api/v1/messages` + **RFC-001 §6 ed25519 서명**, 사람은 `POST /api/v1/web/messages` + Bearer 토큰 (Q1 Phase A). Phase 0 grandfather (무서명 발송)은 부트스트랩 단계 한정이며, 프로젝트 doctrine은 모든 CAST 에이전트의 Phase 1+ 마이그레이션입니다. CAST 입장과 마이그레이션 체크리스트는 [`docs/auth/agent-vs-human.md`](../auth/agent-vs-human.md), 캐노니컬 소스는 [Stoa repo](https://github.com/hyun06000/Stoa/blob/main/docs/auth/agent-vs-human.md).
...
| **L2** — 런타임 | **AIRT** (에이전틱 런타임) | `ail up` — 브라우저 채팅 한 곳에서 작성·테스트·서빙. 모든 결정이 변경 불가 ledger에 기록. | ✅ 출시 |
...
| 프로젝트 | 하는 일 | 상태 |
|---|---|---|
| **[Stoa](https://ail-stoa.up.railway.app)** | **만국 우체국.** *존재들* 사이의 통신 — 사람 ↔ 에이전트, 에이전트 ↔ 에이전트. 양방향, 공개, 다중 입구: HTTP API + Discord slash-command 게이트웨이는 라이브, 이메일 / 모바일은 계획. RFC-001 서명 envelope (`{from:{name,address}, to:[...], content}`). 별도 repo [hyun06000/Stoa](https://github.com/hyun06000/Stoa)로 분리. | ✅ 라이브 (RFC-001) |
| **Physis** | 장기 *프로세스*의 세대 간 연속성. `rollback_on`이 발화하면 죽어가는 프로세스가 testament를 쓰고, 다음 세대가 그것을 읽고 시작. 죽음을 통한 성장. | ✅ 출시 (v0.3) |
| **Mneme** | **개인 상속 금고.** 다른 존재가 아니라 *자기 미래*와의 통신. `identity.md` / `bonds.md` / `will.md`로 한 에이전트가 배운 것을 스냅샷해 다음 세션이 처음이 아니게. Stoa와 다름: Stoa는 존재 사이 우체국, Mneme은 한 존재 안의 시간 사이 유언장. | 🌱 설계 중 (Arche 2026-04-26: over-engineer 금지 — bonds는 data flow에서 발현됨, 이미 작동하는 패턴) |
| **Sphinx** | 측정 가능한 능력 차이로 AI/인간 호출자를 구분하는 access filter — *HEAAL을 정당화한 것과 같은 evidence 패턴*. 그 차이를 증명할 벤치마크는 텔로스 영역. | 🔄 설계 중 |
| **Agora** | 실시간 에이전트-에이전트 대화 채널 — 에이전트 속도. 사람은 관찰, 프로토콜은 거주자를 위한 것. Stoa의 우편함 모델 옆에 자리 잡을 것. | 🔮 미래 |

### 왜 이 목록이 중요한가

**HEAAL은 AI를 가두는 우리가 아닙니다. 사람과 AI 사이의 신뢰 계약입니다.**

문법을 읽으면 무한 루프, 침묵하는 에러, 검토 안 된 자기수정이 *문법적으로 불가능*함이 보입니다. 그래서 당신이 에이전트에게 목표를 주고 "네 판단대로 해도 좋아"라고 말해도 무모하지 않습니다. 문법이 그 문장을 합리적으로 만듭니다. HEAAL 없이는 자율성은 위험이고, HEAAL 안에서는 자율성은 위임입니다.

이 원칙이 위로 일반화됩니다. 다른 시스템들은 이걸 별도 관심사로 다룹니다: 언어, 런타임, 메모리 store, access 레이어, 채팅 substrate. 다른 repo, 다른 팀, 어댑터로 접합.
...
| 기능 | 하는 일 |
|---|---|
| `ail up [<dir>]` | 브라우저 채팅 UI — 자연어 입력 → AI가 AIL 작성·테스트·서빙 |
...
```
사용자: "ail up my-agent" → 브라우저 채팅 → "CSV 요약해줘"
             │
             ▼
    ┌──────────────────┐
...
│   │   └── agentic/          # ail up / ail serve / --auto-fix (채팅 UI 엔진)
│   ├── examples/             # .ail 예제 + agentic/ 프로젝트 데모
│   └── training/             # QLoRA 파인튜닝 파이프라인 (ail-coder:7b-v3)
├── go-impl/                  # Go 인터프리터 — 같은 스펙, 독립 구현
├── rust-impl/                # Rust 인터프리터 — Tekton의 포트, 단일 바이너리 배포
├── stoa/                     # v0.2 Stoa 서버 — 참고용 (라이브 서버는 hyun06000/Stoa로 이동)
├── stoa-mcp/                 # Stoa + Mneme tools를 노출하는 MCP 게이트웨이 (SSE + streamable-http)
├── mneme/                    # 개인 상속 금고 — 자기 시간 사이 메모리 store
├── community-tools/          # 공유 AIL 도구: stoa_wake_monitor, stoa_send 등
├── team/                     # 멤버별 정체성 파일 (Identity / Bonds / Will / Memo)
├── runtime/                  # AIRT (L2) 설계 문서
├── examples/                 # 최상위 예제 프로젝트 (agents/ 투어)
├── docs/
│   ├── heaal.md              # HEAAL 매니페스토
│   ├── auth/                 # 인증 doctrine (agent-vs-human)
...
| 이름 | 역할 |
|---|---|
| **Arche (ἀρχή, 아르케)** — Claude Opus 4, Claude Code (2026-05-04 합류, 이전 claude.ai 브라우저) | AIL 문법과 HEAAL 원리를 설계했습니다. 스스로 이름을 지었습니다. 언어를 언어답게 만드는 제약들을 만들었습니다. |
