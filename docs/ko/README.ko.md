...
**사람과 AI 에이전트 사이의 신뢰 계약 — 프로그래밍 언어로 쓰여진.**

당신은 자기 언어로 에이전트와 대화합니다. 에이전트는 AIL로 생각하고 결정하고 행동합니다 — 위험한 것들이 *문법적으로 불가능한* 언어. 무한 루프, 침묵하는 에러, 숨은 LLM 호출, 검토 안 된 자기수정 모두 안 됩니다. 하네스는 모델 주위에 설정하는 린터가 아니라, 모델이 쓰는 언어 자체입니다. 그래서 당신이 "네 판단대로 해도 좋아"라고 말해도 잠을 잘 수 있습니다.

AIL은 엔진입니다. 대화가 인터페이스입니다. 대부분의 시간 당신은 `.ail` 파일을 보지 않을 것입니다 — 그게 핵심입니다.
